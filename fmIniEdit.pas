...
procedure TfmIniEditor.AfterConstruction;
begin
  inherited;
  Initialize;
...
procedure TfmIniEditor.ConfigSetEditText(Sender: TObject; ACol, ARow: Integer;
  const Value: string);
var
  iIndex: Integer;
begin
...
procedure TfmIniEditor.ConfigValidate(Sender: TObject; ACol, ARow: Integer;
  const KeyName, KeyValue: string);
var
  ItemProps: TItemProp;
  I: Integer;
...
procedure TfmIniEditor.Initialize;
var
  Value: string;
...
procedure TfmIniEditor.WritePosition;
begin
  IniProc(ConfigName, procedure (Ini: TIniFile)
  begin
    TempObjectProc(TStringList.Create, TProc<TObject>(procedure (Reader: TStringList)
...
procedure TfmIniEditor.WndProc(var Message: TMessage);
begin
  inherited;
...
procedure TfmIniEditor.GetValues;
begin
  TempObjectProc(TStringList.Create, TProc<TObject>(procedure (Key: TStringList)
  begin
    Key.Delimiter := '/';
