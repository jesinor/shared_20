...
interface
uses
  Forms, raFunc, ppRTTI, Dialogs, SysUtils, FireDAC.Comp.Client;
...
type
  ...
  TSubTexto = class( TraStringfunction )
  public
        procedure ExecuteFunction(aParams: TraParamList); override;
        class function GetSignature: String; override;
        class function HasParams: Boolean; override;
  ...
  function GetSubTextoRb: string;
  procedure SetSubTextoRb( const Cadena: string );
  ...
  procedure setTablaDatos( FTabla: TFDMemTable );

  var
     FNumero: integer;
     FCad, FTexto, FSubTexto, FTextoMemo, FEmail, FNombreArchivo: string;
     FPesosEnLetras: string;
     FTablaDatos: TFDMemTable;
...
begin
  ...
  try
    // FTablaDatos.Open;
    if not ( FTablaDatos.active ) then
      FTablaDatos.open;
    Datos := FTablaDatos.FieldByName('DENOMINA').value;
  except
    Datos := 'Archivo no ha sido abierto: ' + FTablaDatos.name;
  end
  else
...
begin
  ...
  try
    // FTablaDatos.Open;
    if FTablaDatos.active then
      Datos := FTablaDatos.FieldByName('CUIT').value;
  except
...
begin
  ...
  try
    // FTablaDatos.Open;
    if FTablaDatos.fieldbyname('CONDICIVA').value = 1 then
      FIVA := 'Responsable Inscripto'
    else if FTablaDatos.fieldbyname('CONDICIVA').value = 2 then
...
procedure TSubTexto.ExecuteFunction(aParams: TraParamList);
var
     FCad: string;
begin
  // inherited;
...
class function TSubTexto.GetSignature: String;
begin
     Result := 'function SubTexto: string;';
end;

class function TSubTexto.HasParams: Boolean;
begin
     Result := false;
end;
...
initialization
        ...
        raRegisterFunction( 'SubTexto', TSubTexto );
